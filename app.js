...
function checkSpelling(input, correct) {
  ...
  // 正解が「不快な・窮屈な」のように「・」区切りで複数候補を持つ場合、いずれか一つに一致すればOK
  const candidates = correct.split("・").map(s => s.trim()).filter(Boolean);
  const isSpanish = /[a-zA-Z]/.test(correct);
  if (isSpanish) {
    const normInput = normalizeText(input);
    return candidates.some(c => normalizeText(c) === normInput);
  }
  const trimmedInput = input.trim();
  return candidates.some(c => c === trimmedInput);
}
